#!/usr/bin/env node
/**
 * Synthesizes every sound in the YouTube pitch from scratch: a 120 BPM music
 * bed plus a handful of UI sound effects. No samples, no voiceover — the
 * founder records VO later. The soundtrack is reproducible from this file.
 *
 ...
 *
 * The music reads src/timeline.json so the drop lands on the cut into
 * "Promesa", a short breakdown opens "Pago", and the outro starts with the
 * CTA. If you retime scenes, re-run this script.
 */
...
function buildMusic() {
  ...
  const pago = starts.find((s) => s.id === "Pago").start;
  // Six seconds of air under the "why a card" line, then the groove returns
  // with the wallet. The picture is doing the explaining; the bed steps back.
  const breakdownEnd = pago + 6;
  ...
  for (let b = 0; b < beats; b++) {
    ...
    const inBreakdown = t >= pago - 1e-6 && t < breakdownEnd - 1e-6;
    const inOutroTail = t >= finalHit - 1e-6;

    if (inOutroTail) continue;

    // ---- intro and the payment breakdown: soft mallets, room for the type.
    if (inIntro || inBreakdown) {
  ...
  // Riser + snare-roll into the drop, and a shorter one into the wallet.
  addMono(buf, drop - beat * 4, riser(beat * 4), 0.2);
  for (let i = 0; i < 16; i++) {
    const t = drop - beat * 2 + (i * beat) / 8;
    addMono(buf, t, C, 0.06 + (i / 16) * 0.22, 0);
  }
  addMono(buf, breakdownEnd - beat * 2, riser(beat * 2), 0.16);
